          ...
          Padding(
            ...
            child: BlocBuilder<FamilyBloc, FamilyState>(
              builder: (context, state) {
                ...
                if (state is FamilyLoadSuccess && state.members.isNotEmpty) {
                  // Если у пользователя есть семья
                  final familyMembers =
                      state.members
                          .where((member) => member.id != currentUserId)
                          .toList();
                  ...
                  return DropdownButton<String>(
                    ...
                    onChanged: (value) {
                      // Ничего не делаем, так как только один пункт
                    },
                    underline: Container(height: 2, color: Colors.deepPurple),
                  );
                }
